...
| `macvm` | [nix-darwin](https://github.com/LnL7/nix-darwin) | `aarch64-darwin` | UTM guest VM on the Mac | Sandbox under a separate `aloshy` persona — shares macos's stack with a leaner Homebrew set + MCP gateway trimmed off; activated inside the VM |
