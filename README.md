...
|------|----------|--------|------|
| `m3pro` | macOS via [nix-darwin](https://github.com/LnL7/nix-darwin) | `aarch64-darwin` | Apple Silicon workstation |
...
```bash
darwin-rebuild switch --flake .#m3pro     # macOS
...
```bash
docker pull ghcr.io/ismailkattakath/devcontainers/nix-config:latest
```
...
hosts/          Per-host entry profiles (m3pro.nix, nixbox.nix, nixrpi.nix)
