...
# OWN stdio copy of each server, we run ONE shared instance of each server behind
# sparfenyuk/mcp-proxy — a launchd user agent bound to 127.0.0.1. Clients connect
# over HTTP (SSE) to a single long-lived process per server: shared memory-graph
# state, one cache, no duplicate spawns. Nothing listens off-box (host is
# 127.0.0.1 — no tunnel, no Access). `desktop-commander` is deliberately EXCLUDED
# from the gateway (shell/RCE surface) and stays a per-client stdio server.
#
# SERVER SIDE (this box, 127.0.0.1:${toString gatewayPort})
...
# The ./.mcp.json in this repo is left as-is (project scope) and overlaps by
# name; prune it later if the duplication grates.
{
  pkgs,
  lib,
  config,
