...
#
# `queueSrc` IS AN ARGUMENT, NOT A `../packages/…` IMPORT. A capsule may not use
# a parent-directory path literal (ast-grep/rules/capsule-must-not-reach-out.yml,
# severity error), so ./flake-module.nix passes the package FUNCTION's path DOWN
# — the same shape the cloudflared-connector and keychain-secrets capsules use
# for their `module` argument. It is still `callPackage`d here with stubs, for
# the reason the paragraph above gives: this check needs the function, not the
# ready-made derivation from ./package-graph.nix.
{
  pkgs,
  queueSrc,
}:
let
  ...
  queue = pkgs.callPackage queueSrc {
