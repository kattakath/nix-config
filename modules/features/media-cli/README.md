# media-cli — an in-tree capsule

> Absorbed from the standalone `kattakath/nix-media-cli` flake by ADR-002 wave 5
> ([`docs/monoflake-capsule-adr.md`](../../../docs/monoflake-capsule-adr.md)). That repo is
> archived; this directory is the code, and `flake-module.nix` is the only file anything
> outside it imports.
...
purpose — every CLI reaches the Mac through the module's `home.packages`, built with the
host's own pkgs, and a second perSystem-pkgs copy would be eleven `nix flake show` rows
nothing consumes. `checks.<system>.media-cli-packages` still BUILDS all eleven on every PR,
which is what runs shellcheck over them. The one-line path back is in `flake-module.nix`.
...
MIT — the satellite's own licence, and this repo's. The capsule carries no separate
`LICENSE` file: `LICENSE` at the repo root covers it.
