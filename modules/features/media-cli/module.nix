...
let
  ...
  # knobs this module owns are threaded in HERE, which is the whole point:
  # before the graph was shared, `visionModel` existed on this side only and
  # `ollamaHost` had no build-time path at all, so the flake's `media-describe`
  # and this module's were quietly different derivations.
  inherit
    (import ./package-graph.nix {
      inherit pkgs;
