...
#   TART_VM_SSH_IDENTITY / TART_VM_IP_WAIT — ssh key + ip-wait seconds
{
  ...
  #
  # MANDATORY ARGUMENT, not a default — it was `? ../templates/vanilla-tahoe.pkr.hcl`
  # in the satellite. A capsule may not reach outside itself with a `..` path
  # literal (ast-grep/rules/capsule-must-not-reach-out.yml, severity error), and
  # `packages/` -> `templates/` is a parent traversal even though it stays inside
  # the capsule. The documented fix is the one applied here: the entry file
  # (../flake-module.nix) passes the sibling DOWN. Same value, same store path,
  # same drv.
  defaultTemplate,
}:
let
