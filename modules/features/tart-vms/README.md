# `tart-vms` — the Tart macOS-guest capsule

> **Rehomed.** This was the standalone `github:kattakath/nix-tart-vms` flake
> until **ADR-002 wave 5** absorbed it into this repo as
> `modules/features/tart-vms/` ([`docs/monoflake-capsule-adr.md`](../../../docs/monoflake-capsule-adr.md)).
> History stays in the archived origin repo. The text below is the satellite's
> own README, kept because it is the only prose that explains *why* each piece
> exists — read § Install for the in-tree spelling of every command, and treat
> a `github:kattakath/nix-tart-vms#x` invocation further down as `nix run .#x`.
>
> **Capsule rules that now apply to everything in here:** nothing in this
> directory may reach outside it with a `..` path literal
> (`ast-grep/rules/capsule-must-not-reach-out.yml`, severity `error`), and
> `flake-module.nix` is the only file anything outside imports. That header is
> the authoritative account of what moved and what did not.
...
## Install — IN-TREE (there is no input any more)

Nothing to add to `flake.nix`. The capsule registers itself, so:

| Was (satellite) | Is (in-tree) |
|---|---|
| `nix run github:kattakath/nix-tart-vms#tart-vm -- …` | `nix run .#tart-vm -- …` |
| `packages.aarch64-darwin.tart-vm` | same attribute, on **this** flake |
| `darwinModules.default` / `.tart` | `config.capsuleModules.darwin.tart-vms` |
| `darwinModules.github-runner` / `.runner` | `config.capsuleModules.darwin.tart-github-runner` |
| `darwinModules.gitlab-runner` | `config.capsuleModules.darwin.tart-gitlab-runner` |
...
```nix
imports = [ config.capsuleModules.darwin.tart-github-runner ];
...
```sh
nix run .#tart-gitlab-print-config
