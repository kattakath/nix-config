...
# this repo's raw-served `packages/vast-bootstrap.sh` +
# `packages/templates/provisioner/`. Returns `writeShellApplication`s
# (shellcheck'd by BUILDING them at `nix flake check`), wired as flake apps in
# modules/parts/packages.nix:
#
...
#   }
# In THIS tree they come from `config.fleet.{orgName,repoName,userName}` and
# `self.rev or "main"` — see ../flake-module.nix.
{
  ...
  userName,
  rev,
  # The four scaffold files `vast-init-repo` bakes into a new provisioner repo.
  # Passed IN rather than reached by `./templates/provisioner/…`: two of them are
  # also served over raw HTTP from this repo's `packages/` tree at a path a
  # stored Vast template has already baked into its PROVISIONING_SCRIPT, so they
  # cannot move in here (ADR-002 §4 finding S2). The capsule boundary
  # (ast-grep/rules/capsule-must-not-reach-out.yml) then forbids reaching back
  # OUT at them by path — so the engine hands them down. See
  # `fleet.vastRawServed` in modules/parts/packages.nix.
  provisionerTemplate,
}:
let
  ...
  init-repo = writeShellApplication {
    ...
    text = ''
      ...
      tmp="$(mktemp -d)"
      trap 'rm -rf "$tmp"' EXIT
      cp ${provisionerTemplate.provision} "$tmp/provision.sh"
      cp ${provisionerTemplate.provisionLib} "$tmp/provision-lib.sh"
      cp ${provisionerTemplate.marker} "$tmp/.provisioner-template.json"
      cp ${provisionerTemplate.readme} "$tmp/README.md"
  ...
  # (`scripts-lint` was HERE in the satellite. It lints the raw-served
  # instance-side scripts, which this flake owns rather than this kit, so it
  # moved to ../checks/scripts-lint.nix — the capsule anatomy's home for a
  # check. Nothing else in this file referenced them, and `shellcheck` /
  # `runCommand` left with it.)
in
{
  inherit
    template-apply
