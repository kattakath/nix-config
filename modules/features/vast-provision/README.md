# vast-provision — the Vast.ai GPU-template provisioning capsule

> **Rehomed.** This was the standalone MIT flake
> `github:kattakath/nix-vast-provision`, absorbed into this repo by ADR-002
> wave 4 ([`docs/monoflake-capsule-adr.md`](../../../docs/monoflake-capsule-adr.md)).
> The origin repo is archived and keeps the history. The design document this
> implements is
> [`docs/vastai-template-provisioning.md`](../../../docs/vastai-template-provisioning.md).
>
> **What this capsule may NOT own.** `packages/vast-bootstrap.sh` and
> `packages/templates/provisioner/` stay at the REPO ROOT, not in here: their
> repo paths are baked into every stored Vast template's
> `PROVISIONING_SCRIPT` / `PROVISION_LIB_URL`, so moving them 404s a rented,
> BILLED instance while `nix flake check` stays green. The engine hands them
> down as `config.fleet.vastRawServed` (`modules/parts/packages.nix`).
...
Nothing to install — it is in the tree. From a `nix-config` checkout:

```sh
nix run .#vast-template-apply -- --help
```

or `nix run github:kattakath/nix-config#vast-template-apply`. The six CLIs are
also `packages.aarch64-darwin.vast-*`. The capsule registers them in
`flake-module.nix`; the `apps` wrappers (with this fleet's own
`meta.description` strings) live in `modules/parts/packages.nix`.
...
This fleet. `macos` is the only host that runs any of it — Vast is **not** a
fleet host; the toolkit merely reaches out from the Mac to stand up external
x86_64 cloud GPUs. The full architecture, including the three template modes and
the instance-side boot sequence, is
[`docs/vastai-template-provisioning.md`](../../../docs/vastai-template-provisioning.md).

## License

MIT © Ismail Kattakath — the licence the extracted flake carried, kept with the
code. The repo as a whole is MIT too (`LICENSE`).
