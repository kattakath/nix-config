...
- `modules/` — reusable modules split by platform: `modules/darwin/core.nix`, `modules/linux/nix-ld.nix`, `modules/shared/home.nix`, `modules/nixos/core.nix`, `modules/nixos/cloudflared.nix` (opt-in `services.cloudflared-connector.enable` option; hardened `systemd.services.cloudflared-connector`; token read from `/etc/secrets/cloudflared-token` placed manually after provisioning, never in git; activation script warns if absent), `modules/nixos/litellm.nix` (custom hardened `systemd.services.litellm` for the LiteLLM OpenAI-compatible proxy — no upstream `services.litellm` exists; `services.litellm-proxy.enable` defaults false so it's a graceful no-op until a host opts in; keys via agenix `EnvironmentFile` + `os.environ/VAR` placeholders, never in the store). **All NixOS hosts use mDNS (`<hostname>.local`) for SSH — no SSH tunnels. `nixrpi` has HTTP-only Cloudflare tunnels for service ingress (`litellm.kattakath.com`, `kattakath.com`); `nixarm`/`nixamd` have no tunnels at all.** Per-host tunnels/ingress/DNS live in the Cloudflare account, provisioned by `scripts/cf-one-provision.sh`. Platform branching lives HERE behind `lib.mkIf`, not duplicated across hosts.
...
- **No secrets in Nix:** the store is world-readable. System/service credentials (tunnel tokens, API keys) live at `/etc/secrets/<name>` on each host — placed manually after provisioning, never committed to git. Activation scripts warn at switch time if expected files are absent. **Personal tokens** live in the macOS login Keychain (exported by host-local `~/.zprofile`) or via one-time CLI logins (`gh`/`hf`/`docker`). Never literals in `.nix`.
