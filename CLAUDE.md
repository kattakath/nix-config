...
- `.claude/agents/ci-release-driver.md` — subagent that owns the push→CI→iterate→merge loop against Garnix CI (never merges without approval).
...
- `garnix.yaml` — Garnix hosted CI: builds every flake output across the canonical system triple on native (non-QEMU) builders and reports one status check per output; this is the multi-system CI where the locally-deferred full flake evaluation/build actually runs. (`.github/workflows/` keeps only `build-devcontainer.yml`, `claude*.yml`, and `gitleaks.yml`.)
