...
- `flake.nix` — entry point: pins `nixpkgs` + `home-manager` + `nix-darwin` + `raspberry-pi-nix` + `agenix` + `treefmt-nix` + `git-hooks` inputs; exports `darwinConfigurations."nixcon"` (aarch64-darwin) + `"nixtel"` (x86_64-darwin), `nixosConfigurations."nixarm"` (aarch64-linux), `"nixamd"` (x86_64-linux) and `"nixrpi"` (aarch64-linux), `apps.aarch64-darwin.nixarm-vm` (QEMU+HVF launcher, no UTM needed), plus `packages`/`devShells`/`checks`/`formatter` per system via a `forAllSystems` helper. Username is `ismail`, defined once as a `let` binding.
...
- `modules/` — reusable modules split by platform: `modules/darwin/core.nix`, `modules/linux/nix-ld.nix`, `modules/shared/home.nix`, `modules/nixos/core.nix`, `modules/nixos/cloudflared.nix` (NixOS boot-time loginless Cloudflare TOKEN connector as a hardened `systemd.services.cloudflared-connector`; token via agenix `EnvironmentFile`, NOT `services.cloudflared` upstream which has no token support; a no-op on hosts that don't declare `age.secrets."<host>-tunnel-token"`), `modules/nixos/litellm.nix` (custom hardened `systemd.services.litellm` for the LiteLLM OpenAI-compatible proxy — no upstream `services.litellm` exists; `services.litellm-proxy.enable` defaults false so it's a graceful no-op until a host opts in; keys via agenix `EnvironmentFile` + `os.environ/VAR` placeholders, never in the store). **Only the NixOS hosts (`nixarm`/`nixrpi`/`nixamd`) run the `cloudflared-connector` as SSH tunnel TARGETS; the macOS hosts (`nixcon`/`nixtel`) are tunnel CLIENTS only — no connector, no sshd — reaching the NixOS hosts via the `*.kattakath.com` ProxyCommand in `modules/shared/home.nix`.** Per-host tunnels/ingress/DNS live in the Cloudflare account, provisioned by `scripts/cf-one-provision.sh`. Platform branching lives HERE behind `lib.mkIf`, not duplicated across hosts.
- `packages/` — `docker-image.nix` (minimal baseless runtime image, `dockerTools`), `devcontainer-image.nix` (multi-arch devcontainer image, Linux-only, published to GHCR), `litellm-image.nix` (baseless LiteLLM proxy OCI image, Linux-only; keys via runtime `docker run -e …`), `litellm-compose.nix` (Nix-rendered `docker-compose.yml` for the LiteLLM deploy — `litellm` + a `postgres` backing the Admin UI/virtual keys/Google SSO + `cloudflared`, `pkgs.formats.yaml`), `nixarm-vm.nix` (QEMU+HVF launcher). The `nixarm-image` qcow2 is derived inline in `flake.nix`, not a file here.
...
- **No secrets in Nix:** the store is world-readable. Secrets split by consumer: **system/service** creds (e.g. the per-host cloudflared connector token `<host>-tunnel-token.age`, one line `TUNNEL_TOKEN=…`) use **agenix**, host-key scoped, decrypted at boot and fed to `cloudflared` via `EnvironmentFile` (never argv/store) — this applies to the NixOS tunnel-TARGET hosts only; the macOS hosts run no connector and hold no tunnel token (they are tunnel clients). **personal tokens** are NOT in Nix or git — they live in the macOS login Keychain (exported by host-local `~/.zprofile`) or via one-time CLI logins (`gh`/`hf`/`docker`). agenix was dropped for personal secrets to avoid version-control churn on rotation. Never literals in `.nix`. See `secrets/README`.
...
- [`deploy/litellm/README.md`](deploy/litellm/README.md) + [`infra/cloudflare/litellm.nix`](infra/cloudflare/litellm.nix) (terranix Cloudflare provisioning, applied via `nix run .#cf-litellm-apply` / torn down via `nix run .#cf-litellm-destroy`, OpenTofu; needs a live `CLOUDFLARE_API_TOKEN`) + [`packages/litellm-compose.nix`](packages/litellm-compose.nix) (Nix-rendered `docker-compose.yml`) — the LiteLLM proxy container (+ a `postgres` container backing the Admin UI/virtual keys/SSO) behind a **dedicated** Cloudflare Tunnel at `litellm.kattakath.com`, fronted by Cloudflare Access. Humans log in via **Google Workspace SSO** (`/sso/callback`, restricted to `kattakath.com`); API clients use **virtual keys** (the master key stays server-only, never distributed). This is the **HTTP-ingress + Access** container path (docker-DNS ingress `http://litellm:4000`, Access `kattakath.com` email-domain allow policy — which lets the Google→`/sso/callback` redirect survive Access — plus a service-token policy), distinct from the per-host SSH connector model above; see the §9a comparison in `docs/tunnel-architecture-and-runbook.md`.
