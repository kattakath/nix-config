...
# userName / domainName / accountId are threaded from flake.nix's single sources of
# truth (via _module.args in cfSshConfig), so the SSH login the cert may assert
# tracks the actual `users.users.${userName}` on nixpi, the Access email-domain gate
# tracks `domainName`, and the account id is shared with the tunnel stack — no
# hand-maintained duplicates to drift.
{
  userName,
  domainName,
  accountId,
...
let
  ...
  sshPort = 22;
  allowEmailDomain = domainName; # operator's Google Workspace domain (flake.nix domainName)
