...
- CI is **Garnix** (hosted Nix CI, see `garnix.yaml`) — it builds all flake outputs across the
  three systems (`aarch64-darwin`, `x86_64-linux`, `aarch64-linux`) on NATIVE builders and reports
  one GitHub status check per output plus an `All Garnix checks` aggregate. This is where full
  evaluation actually happens; local hosts often lack `nix`. `build-devcontainer.yml`,
  `gitleaks.yml`, and `claude-config-lint.yml` remain as GitHub Actions workflows.
