...
allowed-tools: Bash(nix flake update:*), Bash(git add:*), Bash(git diff:*), Bash(nix flake check:*)
---
...
4. Run `nix flake check` to confirm the bump still evaluates on `aarch64-darwin` and `aarch64-linux`.
