...
3. **Report** — relay the agent's per-system pass/fail table and final `READY` / `BLOCKED` verdict verbatim. If `nix` is unavailable locally, state clearly that results are syntax-only and full evaluation is CI-deferred to Garnix (see `garnix.yaml`).
