---
description: Stage all .nix files then evaluate the flake across all four target systems via the platform-compiler agent.
argument-hint: "[config]  # optional: nixcon | nixtel | nixarm | nixamd | nixrpi"
...
2. **Cross-platform evaluation** — delegate to the `platform-compiler` agent to evaluate every flake output across `aarch64-darwin`, `x86_64-darwin`, `x86_64-linux`, and `aarch64-linux`.
...
$ARGUMENTS may name a single config (e.g. `nixcon`, `nixtel`, `nixarm`, `nixamd`, `nixrpi`) to scope the evaluation; pass it through to the agent.
