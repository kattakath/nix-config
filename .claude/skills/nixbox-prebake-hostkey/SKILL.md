---
name: nixbox-prebake-hostkey
description: >
  Bake a PINNED SSH host key into the nixbox qcow2 so the Cloudflare tunnel (and any agenix
  ...
  "avoid rekeying every VM", "first-boot working tunnel", or when a fresh nixbox VM's
...
# nixbox prebake host key — first-boot-working tunnel (approach b)
...
- **System agenix is an ACTIVATION-SCRIPT snippet (`agenixInstall`), NOT a systemd unit.** It runs
  ...
  which is why we inject it straight into the ext4 root (no oneshot, no `hosts/nixbox.nix` change).
...
- **No secret in the Nix store, ever.** The pure image (`nix build .#nixbox-image`) contains NO key.
  The private key lives only as agenix ciphertext (`secrets/nixbox-hostkey.age`, encrypted to the
  personal key) and as plaintext **only** in container `/tmp` during injection (shredded after).

## One-time pinning (offline, on the Mac)

```bash
WORK=$(mktemp -d /tmp/nixbox-pin.XXXXXX); chmod 700 "$WORK"
ssh-keygen -t ed25519 -N "" -C "root@nixbox" -f "$WORK/ssh_host_ed25519_key"
...
age -r "$USERKEY" -o secrets/nixbox-hostkey.age "$WORK/ssh_host_ed25519_key"
```

Then in `secrets/secrets.nix`:
- set the `nixbox` recipient to the **pinned public key** (`$PUB`),
- add `"nixbox-hostkey.age".publicKeys = userKeys;`  (**build-time only — never wire into any
...
```bash
TMP=$(mktemp); age -d -i ~/.ssh/id_ed25519 secrets/nixbox-tunnel-creds.age > "$TMP"
age -r "$USERKEY" -r "$PUB" -o secrets/nixbox-tunnel-creds.age "$TMP"
...
```bash
age -d -i "$WORK/ssh_host_ed25519_key" secrets/nixbox-tunnel-creds.age | sha256sum   # must match source
```
...
```bash
# 1. Build the PURE image (no key inside) — see nixbox-utm-prebuild-on-devcontainer:
devcontainer exec --workspace-folder . -- bash -lc 'git add -A && nix build .#nixbox-image'
...
age -d -i ~/.ssh/id_ed25519 secrets/nixbox-hostkey.age > /tmp/hk        # on the Mac
...
devcontainer exec --workspace-folder . -- bash -lc 'cat /tmp/img.qcow2 > /workspaces/nix-config/nixbox-injected.qcow2'
chmod u+w nixbox-injected.qcow2
```

## Boot + verify (zero logins)

Create a UTM VM from `nixbox-injected.qcow2` via **utm-vm-provision §0** (heredoc plist **with the
...
- **Revert to post-boot rekey**: drop `nixbox-hostkey.age`, restore the old comment, use
  **agenix-host-rekey**. The only durable repo change here is the recipient wrapping + the extra .age.

## Cross-references
- **nixbox-utm-prebuild-on-devcontainer** — builds the pure qcow2 this skill injects into.
