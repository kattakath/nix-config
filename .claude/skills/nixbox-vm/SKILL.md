...
name: nixbox-vm
description: >
  Launch the nixbox NixOS VM on macOS via QEMU with Apple HVF acceleration — no UTM required.
  Use when asked to "run nixbox", "boot the nixbox VM", "launch nixbox without UTM", "start nixbox
  in QEMU", or "use nix run .#nixbox-vm". This is the lightweight alternative to utm-vm-provision:
...
# nixbox-vm — QEMU + HVF launcher (no UTM)

## This vs utm-vm-provision

| | **nixbox-vm** (this skill) | **utm-vm-provision** |
|---|---|---|
| Launch method | `nix run .#nixbox-vm` | UTM GUI + `utmctl` |
...
- **Vars copy** — on first run the script copies `edk2-aarch64-vars.fd` into the state dir (`~/.local/state/nixbox-vm/`) for boot-order persistence. If the state dir already has a `vars.fd` from a previous run it is reused.
- **User-mode networking = NAT** — the guest is not directly reachable by IP; only the forwarded port `localhost:2222` works. mDNS (`nixbox.local`) will not resolve until after the Cloudflare Tunnel is active.
...
scp builder:$(ssh builder 'readlink -f /path/to/nix-config/result/*.qcow2') \
    ~/.local/state/nixbox-vm/nixbox.qcow2
```
...
The default disk path is `~/.local/state/nixbox-vm/nixbox.qcow2`; override with `NIXBOX_DISK`.
...
The VM boots with serial output on stdio. Login prompt appears when boot completes. The SSH daemon is enabled in `hosts/nixbox.nix` — it starts automatically.

### Environment overrides

| Variable | Default | Effect |
|---|---|---|
| `NIXBOX_DISK` | `~/.local/state/nixbox-vm/nixbox.qcow2` | Path to the qcow2 disk image |
| `NIXBOX_MEMORY` | `2048` | RAM in MiB |
| `NIXBOX_CPUS` | `4` | vCPU count |

Example — boot with 8 GiB and 6 CPUs using a custom disk:

```bash
NIXBOX_DISK=~/vms/nixbox-dev.qcow2 NIXBOX_MEMORY=8192 NIXBOX_CPUS=6 nix run .#nixbox-vm
```

## Step 3 — First-boot SSH and agenix rekey

On first boot the host generates a fresh `ssh_host_ed25519_key`. The `nixbox-tunnel-creds.age` secret is encrypted only to the personal key placeholder — `services.cloudflared` will fail to decrypt and will not start. This is expected.
...
Then run the **agenix-host-rekey** skill to add this key as an age recipient and re-encrypt `nixbox-tunnel-creds.age`. That skill covers the full flow: editing `secrets/secrets.nix`, re-encrypting, committing, and activating on the host.
