...
### Naming: one rule, because four shapes is what happens otherwise

Objects were added to `mcp-public.nix` one at a time, each copying whatever already existed
nearby. The result, by 2026-09-12, was two prose names for the two origin apps, IdP pinning on
one kind of app but not the other, and the gateway's origin app hand-written as a literal while
the external ones were generated — so nothing forced the two to agree, and they drifted
immediately.

The rules now live at the top of `infra/cloudflare/mcp-public.nix`:

| Object | Rule |
|---|---|
| registration id | `gw-<name>` on the gateway, `ext-<name>` on its own origin |
| resource key | the id with `-` → `_`, prefixed `origin_` / `portal_` by layer |
| **Access application name** | **its destination** — the hostname, or the server id |

The third is the one that shows: in the dashboard the "Application name" and "Destinations"
columns now read identically for every row this module owns, and the "Type" column is what says
which layer it is. Nothing is a prose name that has to be invented or kept in sync.

The structural half matters more than the names — every object is generated by the same `map`
over one `published` list, and everything genuinely shared sits in `accessAppCommon` exactly
once. Drift is no longer something to remember not to do.
