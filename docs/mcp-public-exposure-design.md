...
fleet owns. The "Type" column is what says which layer it is — prose names were repeating that,
and had to be kept in sync by hand.

There is deliberately **no `gw-` / `ext-` prefix**. It encoded *which process serves a server* —
the macos mcp-proxy or a Worker — which stopped being visible or relevant once everything moved
behind the one hostname. A client cannot tell the difference and must not depend on it. Dropping
it cost one replace of each registration (`id` is ForceNew) and one client reconnect.

`SSO App`, `App Launcher` and `Warp Login App` are Cloudflare's own system applications. They are
not ours to rename — the same carve-out `.claude/rules/launchd-naming.md` makes for upstream
launchd units.

**How it got inconsistent:** objects were added one at a time, each copying whatever sat next to
it, and the gateway's origin app was a hand-written literal while the external ones were
generated — so nothing forced them to agree. Everything now comes from one `published` list
through `originApp` / `portalApp`, which share `accessAppCommon`. Drift is no longer something to
remember not to do.
