...
Write `config.json` into the brag data directory — `$BRAG_DATA_DIR` (a locally version-controlled, deliberately REMOTE-LESS repo; set once in nix-config `home.nix` `home.sessionVariables`, NOT this Nix-managed skill dir). See [config.json.example](config.json.example) for the schema.
...
BRAG_DATA_DIR="${BRAG_DATA_DIR:?set once in nix-config home.nix home.sessionVariables — the remote-less ~/Developer/local/brags repo}"
