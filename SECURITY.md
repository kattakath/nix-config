...
- **System / service credentials** (today, just `nixpi`'s Cloudflare Tunnel connector token) live as a plain, root-only file at `/etc/secrets/<name>` on the host — placed manually after provisioning, never committed to git. There is no encryption step (no agenix, no sops) in this repo; the file is simply absent from version control.
