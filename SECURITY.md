...
- **System / service credentials** (today: exactly one — `nixpi`'s Cloudflare Tunnel connector token) are committed **encrypted** with [agenix](https://github.com/ryantm/agenix) — recipient declared in `secrets/secrets.nix` (pure age/SSH, no `ssh-to-age`), ciphertext in `secrets/cloudflared-token.age`. This token is encrypted to the **operator's key alone**, so agenix is an **operator-only vault**: the operator decrypts it on the Mac and plants it on the SD card's FAT `FIRMWARE` partition, from where `nixpi` copies it into a root-only `/run` file at boot — it is **never** host-decrypted on `nixpi` (a fresh SD flash rotates the host key, which would break host-key decryption and kill the only remote path in). No secret is host-decrypted into `/run/agenix/<name>` anymore; the plaintext never touches git or the world-readable store. (agenix replaced sops-nix on 2026-07-08. The `cloudflared-connector` module still defaults to an operator-placed `/etc/secrets/cloudflared-token` for hosts that don't opt into this vault, but `nixpi` overrides it to the FIRMWARE-planted `/run` file.)
