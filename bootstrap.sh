#!/usr/bin/env bash
#
# bootstrap.sh — the ONE part of Mac setup/recovery that cannot live behind Nix.
#
...
# dependencies. It does the irreducible minimum:
#
...
{ # ==== download-completeness guard =========================================
  # bash parses to the matching `}` at EOF BEFORE executing anything, so a
  # truncated `curl | bash` dies with a syntax error and runs NOTHING — no half
  ...
  # read below comes from /dev/tty, never stdin.
  ...
  # is active), and any age passphrase is read by age itself from /dev/tty, so it
  ...
  has_gui() {
    [ -n "${SSH_TTY:-}" ] && return 1
    launchctl managername 2>/dev/null | grep -q Aqua
  }
  ...
  confirm() { # $1 = question. GUI dialog if we have one, else a /dev/tty prompt.
    if has_gui; then
      /usr/bin/osascript -e \
    ...
    [ "$HAVE_TTY" -eq 1 ] || return 1 # headless → safe "No"
    printf '    %s [y/N] ' "$1" >/dev/tty
    local reply
    read -r reply </dev/tty # NOT stdin (that is the curl stream)
    case "$reply" in
